...
try:
    from . import models, training_utils
    from .training_utils import ConstantWarmupScheduler, CosineWarmupScheduler
except ImportError:
    import models
...
class RosettaTask(pl.LightningModule):
    ...
    def add_model_specific_args(parent_parser):
        ...
        p.add_argument(
            ...
            type=str,
            default="constant",
            choices=["constant", "warmup_cosine_decay", "warmup_constant"],
        )
        p.add_argument(
            "--warmup_steps",
            type=float,
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.model = models.Model[model_name].cls(
            num_tasks=num_tasks,
        ...
        self.test_pearson_collection = nn.ModuleList(
    ...
    def test_step(self, data_batch, batch_idx):
        ...
        for task_num in range(self.num_tasks):
            ...
            self.log(
                "test/pearson_{}".format(task_name),
                self.test_pearson_collection[task_num],
                on_step=False,
                on_epoch=True,
            )
        ...
        if self.lr_scheduler == "warmup_cosine_decay":
            ...
            lr_scheduler_config = {
                "scheduler": CosineWarmupScheduler(
                    optimizer, warmup=ws, max_iters=training_steps
                ),
                "interval": "step",
            }
...
class DMSTask(pl.LightningModule):
        ...
        **kwargs,
        ...
        if num_tasks > 1:
            raise NotImplementedError(
                "DMSTask currently only supports 1 task "
    ...
    def test_step(self, data_batch, batch_idx):
        ...
        self.test_spearman(
            torch.squeeze(outputs, dim=-1), torch.squeeze(labels, dim=-1)
        )
    ...
    def configure_optimizers(self):
        ...
        if hasattr(self, "hparams") and self.hparams is not None:
            unfreeze_backbone_at_epoch = self.hparams.get(
                "unfreeze_backbone_at_epoch", None
            )

        optimizer_config = training_utils.OptimizerConfig(
            self.optimizer,
            self.weight_decay,
            self.learning_rate,
            self.lr_scheduler,
        ...
        return optimizer_config.get_optimizer_config(
            trainable_parameters, self.trainer.estimated_stepping_batches
        )
