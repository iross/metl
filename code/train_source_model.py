...
class ModelCheckpoint(pl.callbacks.ModelCheckpoint):
    ...
    def load_state_dict(self, state_dict):
        ...
        if not os.path.isabs(dirpath_from_ckpt):
            ...
            suffix = os.path.commonprefix(
                [state_dict["dirpath"][::-1], self.dirpath[::-1]]
            )[::-1]
            if suffix != self.dirpath:
                ...
                self.kth_best_model_path = self.to_rel(
                    state_dict.get("kth_best_model_path", self.kth_best_model_path)
                )
                self.kth_value = state_dict.get("kth_value", self.kth_value)
                self.best_k_models = {
                    self.to_rel(k): v
                    for k, v in state_dict.get(
                        "best_k_models", self.best_k_models
                    ).items()
                }
                self.last_model_path = self.to_rel(
                    state_dict.get("last_model_path", self.last_model_path)
                )
...
def init_loggers(
    ...
    wandb_project: str,
    version: int,
) -> Union[tuple[WandbLogger, CSVLogger], CSVLogger]:
    # set up loggers for training
    csv_logger = CSVLogger(save_dir=log_dir, name="", version=version)
...
def get_encoding(args):
    if args.encoding == "auto":
        ...
        if args.model_name in ["linear", "fully_connected", "cnn"]:
            ...
            raise ValueError(
                "unknown encoding for model name: {}".format(args.model_name)
            )
    else:
...
def main(args):
    ...
    if torch.cuda.device_count() > 1:
        print(
            "Detected {} CUDA devices, setting strategy to DDP".format(
    ...
    if os.getenv("LOCAL_RANK", "0") == "0":
        ...
        utils.save_args(
            vars(args), join(log_dir_version, "args.txt"), ignore=["cluster", "process"]
        )

    elif (
        "PL_LOG_DIR" in os.environ
        and "PL_UUID" in os.environ
        and "PL_VERSION" in os.environ
        and "PL_LOG_DIR_VERSION" in os.environ
    ):
        ...
        raise ValueError(
            "PL_LOG_DIR or PL_UUID or PL_VERSION or PL_LOG_DIR_VERSION"
            " environment variables not set on rank {}".format(
    ...
    loggers = init_loggers(
        log_dir, my_uuid, args.use_wandb, args.wandb_online, args.wandb_project, version
    )

    # set up the datamodule
    encoding = get_encoding(args)
    dm = RosettaDataModule(
        ds_fn=args.ds_fn,
        encoding=encoding,
    ...
    # set up callbacks
    callbacks = [
        BestMetricLogger(metric="val_loss", mode="min"),
    ...
    if args.condor_checkpoint_every_n_epochs > 0:
        condor_stopping_callback = CondorStopping(
            every_n_epochs=args.condor_checkpoint_every_n_epochs
        )
    ...
    trainer = pl.Trainer(
        ...
        devices=devices,
        strategy=trainer_strategy if trainer_strategy is not None else "auto",
    ...
    if strategy == "ddp":
        ...
        if trainer.is_global_zero:
            ...
            try:
                model = tasks.RosettaTask.load_from_checkpoint(
                    checkpoint_callback.best_model_path
                )
            except:
                print(
                    f"Couldn't find {checkpoint_callback.best_model_path}. Trying to load from {ckpt_path} instead."
                )
        ...
        try:
            ...
            print(
                f"Couldn't find best model path. Trying to load from {ckpt_path} instead."
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--condor_checkpoint_every_n_epochs",
    ...
    parser.add_argument("--wandb_project", type=str, default="metl")
    parser.add_argument(
        "--wandb_log_grads",
        default=False,
        action="store_true",
    ...
    parser.add_argument(
        "--model_name", type=str, choices=[m.name for m in list(models.Model)]
    )
