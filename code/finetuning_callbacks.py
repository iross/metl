    ...
    def on_fit_start(
        ...
        try:
            ...
            raise MisconfigurationException(
                "The LightningModule should have a nn.Module `{}` attribute".format(
        ...
        if filter_unique_params:
            ...
            for param in params:
                if param not in unique_params:
                    ...
                    print(
                        "Filtered out a duplicate parameter when unfreezing the backbone"
                    )
            ...
            if unique_params_list:
                optimizer.add_param_group(
                    {"params": unique_params_list, "lr": params_lr / denom_lr}
                )
        elif params:
...
class BackboneFreezer(Callback):
    ...
    def setup(
        self, trainer: "pl.Trainer", pl_module: "pl.LightningModule", stage: str = "fit"
    ) -> None:
        try:
            backbone = get_module_by_name(pl_module, self.backbone_access_string)
        except AttributeError:
